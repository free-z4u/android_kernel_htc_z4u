...
/** comment codes about communicate with modem by medea
 * this solution is used to resolve off network in modem side.
 * We can't find this issue so comment this solution temporarily.
 * if any problem please contact medea_zhou@htc.com
**/
# if 0
...
#define DMC_FREQ_NORMAL_SCENE			0x0 //whole system in wakeup mode.
#define DMC_FREQ_EARLYSUSPEND_SCENE			0x1 //system in earlysuspend mode.
...
/*
 * This function fix DDR freq to max.
 * @enable:		param 1 fix the DDR frequency to max and praram 0 resume DDR to automatic switch frequency.
 */
...
//	unsigned char cp_req;
	...
	if(time_before(jiffies, boot_done)){
		...
		//if boot mode is power_test, don't need to hold wake lock.
		if(4 != boot_mode) {
			if(!boot_done_init) {
	...
	//printk("*** %s, jiffies:%u, data->last_jiffies:%u ***\n", __func__, jiffies, data->last_jiffies);
	...
	//move by medea to improve LCD performance.
...
/*
 * This is inner function for sprd DDR frequency driver.
 *
 * - Must hoding lock field's spinlock in dmcfreq_data struct
 * - before call this function. Or it will cause unknown cirtical system errors.
 */
static void inline scxx30_set_max(struct dmcfreq_data *data)
{
	unsigned long max;
...
/************ early suspend to resolve LCD which is flickering  *****************/
