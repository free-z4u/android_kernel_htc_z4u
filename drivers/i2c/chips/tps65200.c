...
int tps_set_charger_ctrl(u32 ctl)
{
	...
	switch (ctl) {
	case POWER_SUPPLY_DISABLE_CHARGE:  //0
		...
		regh |= 0x83;
		tps65200_vdpm_chg = 0;
		tps65200_i2c_write_byte(regh, 0x03); /* VDPM = 4.44V */
#endif /* SET_VDPM_AS_444 */

		/* cancel CHECK_CHG alarm */
		alarm_cancel(&tps65200_check_alarm);
		break;
	case POWER_SUPPLY_ENABLE_SLOW_CHARGE:  //1
	case POWER_SUPPLY_ENABLE_WIRELESS_CHARGE:  //4
		tps65200_i2c_write_byte(0x29, 0x01);
		tps65200_i2c_write_byte(0x2A, 0x00);
#if SET_VDPM_AS_444
		regh = 0x83; /* VDPM = 4.44V */
#else  /* SET_VDPM_AS_444 */
...
#endif /* SET_VDPM_AS_444 */
		break;
	case POWER_SUPPLY_ENABLE_FAST_CHARGE:  //2
...
#endif
		if(is_high_current_charging_avaialable())
			regh = 0xA0;  //set battery voltage to 4.14V
		tps65200_i2c_write_byte(regh, 0x02);
	...
	case CLEAR_LIMITED_CHG:
		...
		break;
	case CHECK_CHG:
		i2c_status = tps65200_i2c_read_byte(&regh3, 0x0E);//tps65200_dump_register();
		if (i2c_status == -5) {
	...
	case POWER_SUPPLY_ENABLE_INTERNAL:
		...
		tps65200_i2c_write_byte(0x19, 0x02);  //For OTG func, 100mA, and 5.0V
...
static irqreturn_t chg_stat_handler(int irq, void *data)
{
	pr_tps_info("interrupt chg_stat is triggered. "
			"chg_stat_enabled:%u\n", chg_stat_enabled);
	if (/*chg_stat_enabled &&*/ tps65200_suspended == false){
			...
			const struct i2c_device_id *id)
{
	...
	if (pdata->gpio_chg_stat > 0) {
		...
		rc = request_irq( gpio_irq/*pdata->gpio_chg_stat*/,
				chg_stat_handler,
				IRQF_TRIGGER_RISING,
				"chg_stat", NULL);
