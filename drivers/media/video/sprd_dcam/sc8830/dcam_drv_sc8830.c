...
int32_t dcam_set_clk(enum dcam_clk_sel clk_sel)
{
	...
	case DCAM_CLK_NONE:
		if (s_dcam_clk) {
			...
			clk_put(s_dcam_clk);
//HTC_START Qingyuan set clk to NULL after disable clk ,to avoid kernel warning
			s_dcam_clk = NULL;
//HTC_END
		}
		return 0;
...
void _dcam_wait_sof()
{
	...
	//usleep(100*1000);
	#endif
       //s_path_stop_sema = __SEMAPHORE_INITIALIZER(s_path_stop_sema, 0);
}
		...
		if (DCAM_YUV_ADDR_INVALIDE(p_addr->yaddr, p_addr->uaddr, p_addr->vaddr)) {
			...
			if (path->output_frame_count > DCAM_PATH_2_FRM_CNT_MAX - 1) {
				DCAM_TRACE("DCAM DRV: Path 2 Error, DCAM_PATH_OUTPUT_ADDR, i=%d, y=0x%x, u=0x%x, v=0x%x \n",
					path->output_frame_count, p_addr->yaddr, p_addr->uaddr, p_addr->vaddr);
				rtn = DCAM_RTN_PATH_FRAME_TOO_MANY;
#ifndef CONFIG_MACH_Z4TD // from arch/arm/mach-sc/include/mach/__memlayout-cp5dug.h
...
//HTC_START Qingyuan clean path2 isr flag if isr_flag > 10
			path2_isr_flag ++;
...
//HTC_START Qingyuan reset isr_flag
...
//HTC_START Qingyuan add for path0 endian sel
	dcam_glb_reg_mwr(DCAM_ENDIAN_SEL, BIT_5 | BIT_4, path->data_endian.y_endian << 4, DCAM_ENDIAN_REG); // endian sel
	//   endain_sel  -> 0：B0，B1，B2，B3  little endian
	//   endain_sel  -> 1：B3，B2，B1，B0
	//   endain_sel  -> 2：B2，B3，B0，B1  little endian
	//   endain_sel  -> 3：B1，B0，B3，B2
//HTC_END
