...
extern int front_main; //main 0; front 1
typedef enum {
	SENSOR_MAIN = 0,
	SENSOR_SUB,
	SENSOR_ATV = 5,
...
LOCAL void Sensor_detect()
{
	...
	{
             gpio_fun_sel(REG_PIN_KEYIN2, BITS_PIN_AF(3), BIT_PIN_WPU);     //step 1: configure to GPIO, and set pull up.
             GPIO_SET(201, 1);
             ...
             if(gpio_get_value(201) == 0)
              ...
              //Set GPIO 201 as output low
             GPIO_SET(201, 0);
             gpio_fun_sel(REG_PIN_KEYIN2, BITS_PIN_AF(3), BIT_PIN_NUL);     //step 1: configure to GPIO, and set pull up.
	  }
...
static pinmap_t pinmap_camera_init[] = {
	...
	{REG_PIN_LCD_D19,             BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_Z},//GPIO 127
	//{REG_PIN_KEYIN2,              BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_Z},//GPIO 201
};

static pinmap_t pinmap_camera_exit[] = {
	//{REG_PIN_SPI0_DO,             BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPU|BIT_PIN_SLP_WPU|BIT_PIN_SLP_IE},//GPIO 68
	//{REG_PIN_SPI0_CSN,            BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_OE},//GPIO 67
	{REG_PIN_SPI0_DI,             BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE},//GPIO 69
	//{REG_PIN_SPI0_CLK,            BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_NUL|BIT_PIN_SLP_NUL|BIT_PIN_SLP_OE},//GPIO 70
	{REG_PIN_CCIRCK1,             BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE}, //GPIO 172: rawchip int0
	{REG_PIN_CCIRHS,              BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(3)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE}, //GPIO 174: rawchip int1
	{REG_PIN_KEYOUT2,             BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(0)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE},//GPIO 193: main camera ID
	//{REG_PIN_KEYIN2,              BIT_PIN_NULL|BITS_PIN_DS(1)|BITS_PIN_AF(0)|BIT_PIN_WPD|BIT_PIN_SLP_WPD|BIT_PIN_SLP_IE},//GPIO 201: front camera ID
...
static void camera_pin_exit(void)
{
	...
	//fix after shark spi ready
	//GPIO_SET(67,1);//spi pin 67
	//GPIO_SET(68,0);//spi pin 68
	//GPIO_SET(70,0);//spi pin 70

	GPIO_SET(SUB_CAMERA_RESET,0);//SUB_CAMERA_RESET pin
	GPIO_SET(MAIN_CAMERA_PD,0);//MAIN_CAMERA_PD pin
	GPIO_SET(CAMERA_SEL,0);//CAM_SEL
	GPIO_SET(171,0);//Yushan_Reset_PIN pin
	GPIO_SET(173,0);//GPIO 173: raw_mclk_cpu
	GPIO_SET(CAM_VCM_PD,0);//GPIO 177: VCM_PWD
	GPIO_SET(CAM_IOVDD_EN,0);//GPIO 178: CAM_IOVDD_EN
	GPIO_SET(CAM_AVDD_EN,0);//GPIO 179: CAM_AVDD_EN
	GPIO_SET(CAM_VCMVDD_EN,0);//GPIO 183: CAM_VCM-EN
	GPIO_SET(184,0);//GPIO 184: RAWCHIP_1V2
	GPIO_SET(185,0);//GPIO 185: RAWCHIP_1V8
		...
		SENSOR_PRINT("SENSOR: _Sensor_K_PowerDown -> main: power_down %d\n",
			...
			case SENSOR_MAIN:
				{	printk(" %s : Sensor_K_GetCurId = SENSOR_MAIN\n",__func__);
					if(1 == power_level)
					{	GPIO_SET(CAM_VCM_PD, power_level);//CAM_VCM_PD
			...
			case SENSOR_SUB:
				{	printk(" %s : Sensor_K_GetCurId = SENSOR_SUB\n",__func__);
				      ...
				      //Set GPIO as input
				      //gpio_request(201, "main camera");
				      GPIO_SET(CAMERA_SEL, 1);
	  
//HTC_START Qingyuan front camera porting
#if defined(CONFIG_MACH_Z4DTG) || defined(CONFIG_MACH_Z4TD)
					GPIO_SET(SUB_CAMERA_RESET, power_level);//for s5k6a2
					//GPIO_SET(135, 0);	
                                 if(power_level) {
                                     //msleep(10);
                                     Sensor_detect();
...
#elif defined (CONFIG_MACH_CP5VEDWG)|| defined (CONFIG_MACH_CP5VEDTU)
					GPIO_SET(SUB_CAMERA_RESET,power_level);//for ov9760
#else
					GPIO_SET(SUB_CAMERA_RESET,!power_level);//for ov2722
...
{
	printk("cammot_val %d\n", cammot_val);
	//htc_cam_start, clean, power timing
	GPIO_SET(171,0);//Yushan_Reset_PIN pin
	//htc_cam_end, clean, power timing
	//htc_cam_start, clean, power gpio config 
	if(SENSOR_VDD_CLOSED == cammot_val)
               {camera_pin_exit();}
	else
               {camera_pin_init();}
	//htc_cam_end, clean, power gpio config
	return 0;
}
LOCAL int _Sensor_K_SetVoltage_AVDD(uint32_t avdd_val)
{
	int err;
	//htc cam start -- z4td camera power
...
LOCAL int _Sensor_K_SetMCLK(uint32_t mclk)
{
	...
	if(mclk){
		...
		//printk("00000000000000000000000000000000000000000000000\n");
		//printk("00000000000000000000000000000000000000000000000\n");
		//printk("00000000000000000000000000000000000000000000000\n");
		//printk("00000000000000000000000000000000000000000000000\n");
		//printk("00000000000000000000000000000000000000000000000\n");
			  ...
			  unsigned long arg)
{
	...
	case SENSOR_IO_I2C_WRITE:
		{	//printk(" SENSOR_IO_I2C_WRITE called \n");
			SENSOR_REG_BITS_T reg;
			ret = copy_from_user(&reg, (SENSOR_REG_BITS_T *) arg, sizeof(SENSOR_REG_BITS_T));
	...
	case SENSOR_IO_I2C_WRITE_EXT:
		{	//printk(" SENSOR_IO_I2C_WRITE_EXT called \n");
			SENSOR_I2C_T i2cTab;
