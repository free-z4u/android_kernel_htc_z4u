...
void sx86XX_process_interrupt(struct sx86XX *this,u8 nirqlow)
{
    ...
    if ((status & RESET_AND_COMPDONE_IRQ) == RESET_AND_COMPDONE_IRQ)
        return;

    /* Only look at release interrupt if we are not in startup detection */
    if (((status & FAR_IRQ) == FAR_IRQ) && (this->inStartupTouch == true))
        return;

    /* Reset the flag that we are in startup detection */
