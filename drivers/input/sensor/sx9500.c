...
#define PATH_CAPSENSOR_CAL_CS0 "/sns/capsensor_cal_cs0.dat"
#endif
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
    { .reg = SX9500_CPS_CTRL0_REG,    .val = 0x2D, },
#else
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
    { .reg = SX9500_CPS_CTRL6_REG,    .val = 0x05, },
#else
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
    .defaultSecondStartupMainSensor = SENSORSEL_CS0, /* Main2 */
#endif
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
    sprintf(buf, "%d %02x %02x", val, msByte, lsByte+1);
#else
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
static int write_calibration_data_cs0(struct sx86XX *this, s32 val, u8 msByte, u8 lsByte)
{
    int fd;
    int ret = 0;
    char buf[50];
    ...
    set_fs(KERNEL_DS);
    fd = sys_open(PATH_CAPSENSOR_CAL_CS0, O_WRONLY|O_CREAT, 0664);

    if (fd >= 0) {
        sys_write(fd, buf, sizeof(buf));
        sys_fsync(fd); //ensure calibration data write to file system
        sys_close(fd);
        sys_chmod(PATH_CAPSENSOR_CAL_CS0, 0664);
        set_fs(old_fs);
    }
    else {
        ret++;
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
static void read_calibration_data_cs0(struct sx86XX *this, s32 *nMargin, u8 *msByte, u8 *lsByte)
{
    int fd;
    //int ret = 0;
    int len = 0;
    ...
    *lsByte = (u8) lsByte_Offset;
    ...
    fd = sys_open(PATH_CAPSENSOR_CAL_CS0, O_RDONLY, 0);
    if(fd >= 0) {
        len = sys_read(fd, read_buf, sizeof(read_buf));
        dev_dbg(this->pdev, "cap sensor calibration file size is = %d\n", len);
        if(len <= 0)
                     ...
                     const char *buf, size_t count)
{
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
    unsigned char mainSensor_cs0;
    u8 capMain_cs0_Offset_msByte = 0;
    u8 capMain_cs0_Offset_lsByte = 0;
    s32 capMain_cs0[COLLECT_NUM];
    s32 avg_capMain_cs0 = 0;
    s32 capMargin_cs0 = 0;
#endif
    ...
    if (this && (pDevice = this->pDevice) && !running_cal_or_reset) {
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        unsigned char conv_mainSensor_cs0;
#endif
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        mainSensor_cs0 = (unsigned char)pDevice->ptouchCheckParameters->defaultSecondStartupMainSensor;
#endif
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        conv_mainSensor_cs0 = conv_sensorsel_to_sensoren(mainSensor_cs0);
#endif
        conv_refSensor = conv_sensorsel_to_sensoren(refSensor);
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        write_RegProxCtrl0 = (unsigned char) ((cal_scanperiod << 4) | (conv_mainSensor | conv_mainSensor_cs0 | conv_refSensor));
#else
        ...
        // To sort the collected value. 2 max & 2 min values are excluded.
        for (i = 0; i < COLLECT_NUM; i++) {
            capMain_cs0[i] = calculate_CSx_rawdata(this, mainSensor_cs0);
            dev_dbg(this->pdev,"capMain_cs0[%d] = %d\n", i, capMain_cs0[i]);
            msleep(30);
        }
        read_register(this, SX9500_OFFSETMSB_REG, &capMain_cs0_Offset_msByte);
        read_register(this, SX9500_OFFSETLSB_REG, &capMain_cs0_Offset_lsByte);

        sort(capMain_cs0, COLLECT_NUM, sizeof(s32), cmp_rawdata, NULL);
        for (i = FILTER_NUM; i < COLLECT_NUM - FILTER_NUM; i++)
            avg_capMain_cs0 += capMain_cs0[i];
        avg_capMain_cs0 = avg_capMain_cs0 / (COLLECT_NUM - (FILTER_NUM * 2));
        dev_dbg(this->pdev,"avg_capMain_cs0 = %d\n", avg_capMain_cs0);
#endif
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        capMargin_cs0 = avg_capMain_cs0 - avg_capRef + min_threshold;
#endif
        ret = write_calibration_data(this, capMargin, capMain_Offset_msByte, capMain_Offset_lsByte);
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        ret = write_calibration_data_cs0(this, capMargin_cs0, capMain_cs0_Offset_msByte, capMain_cs0_Offset_lsByte);
#endif
                     ...
                     struct device_attribute *attr, char *buf)
{
    ...
    s16 ref_Useful_CSx = 0;
    s16 ref_Avg_CSx = 0;
    s16 ref_Diff_CSx = 0;
    u16 ref_Offset_CSx = 0;
...
#if defined(CONFIG_MACH_MSM8926_E8LTE)
        // Select Main2 Sensor
        write_register(this, SX9500_SENSORSEL_REG, mainSensor2);

        read_register(this, SX9500_USEMSB_REG, &msByte);
        read_register(this, SX9500_USELSB_REG, &lsByte);
        Useful_CSx2 = (s16)((msByte << 8) | lsByte);
        msByte = 0; lsByte = 0;
        read_register(this, SX9500_AVGMSB_REG, &msByte);
        read_register(this, SX9500_AVGLSB_REG, &lsByte);
        Avg_CSx2 = (s16)((msByte << 8) | lsByte);
        msByte = 0; lsByte = 0;
        read_register(this, SX9500_DIFFMSB_REG, &msByte);
        read_register(this, SX9500_DIFFLSB_REG, &lsByte);
        Diff_CSx2 = (s16)((msByte << 8) | lsByte);
        if (Diff_CSx2 > 4095)
            Diff_CSx2 -= 8192;
        read_register(this, SX9500_OFFSETMSB_REG, &msByte);
        read_register(this, SX9500_OFFSETLSB_REG, &lsByte);
        Offset_CSx2 = (u16)((msByte << 8) | lsByte);
#endif

        //-------------------temp--------------------
        // Select Reference Sensor
        write_register(this, SX9500_SENSORSEL_REG, refSensor);

        read_register(this, SX9500_USEMSB_REG, &msByte);
        read_register(this, SX9500_USELSB_REG, &lsByte);
        ref_Useful_CSx = (s16)((msByte << 8) | lsByte);
        msByte = 0; lsByte = 0;
        read_register(this, SX9500_AVGMSB_REG, &msByte);
        read_register(this, SX9500_AVGLSB_REG, &lsByte);
        ref_Avg_CSx = (s16)((msByte << 8) | lsByte);
        msByte = 0; lsByte = 0;
        read_register(this, SX9500_DIFFMSB_REG, &msByte);
        read_register(this, SX9500_DIFFLSB_REG, &lsByte);
        ref_Diff_CSx = (s16)((msByte << 8) | lsByte);
        if (ref_Diff_CSx > 4095)
            ref_Diff_CSx -= 8192;
        read_register(this, SX9500_OFFSETMSB_REG, &msByte);
        read_register(this, SX9500_OFFSETLSB_REG, &lsByte);
        ref_Offset_CSx = (u16)((msByte << 8) | lsByte);
    ...
    if (this) {
        ...
        if (likely((pDevice = this->pDevice) != NULL)) {
            ...
            touchCheckWithReferenceSensor(this,
                (unsigned char)pDevice->ptouchCheckParameters->defaultSecondStartupMainSensor,
            ...
            touchCheckWithReferenceSensor(this,(unsigned char)0x02, (unsigned char)0x03); // 0x03 ref
#else
            touchCheckWithReferenceSensor(this,(unsigned char)0x02, (unsigned char)0x00);
#endif
        }
 ...
 */
static void touchCheckWithReferenceSensor(struct sx86XX *this,
                                          unsigned char mainSensor,
                                          unsigned char refSensor) 
{
    s32 cal_Margin = -1;
...

    {
        ...
        for (i = 0; i < 5 ; i++) {
            // Select Main Sensor
            write_register(this, SX9500_SENSORSEL_REG, SENSORSEL_CS2);
    ...
    {
        write_register(this, SX9500_SENSORSEL_REG, (u8)mainSensor);
        read_calibration_data_cs0(this, &cal_Margin, &cal_Offset_msByte, &cal_Offset_lsByte);
