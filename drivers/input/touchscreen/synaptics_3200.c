...
static void synaptics_ts_finger_func(struct synaptics_ts_data *ts)
{
	...
	if (ts->package_id < 3400){/*read finger number*/
	...
	if (ret < 0) {
		...
		/*read touch raw data, only for syn3201/syn3202*/
		if (ts->package_id < 3400){
			for (i = 0; i < ts->finger_support; i++) {
	...
	unsigned long status, void *unused)
{
	...
	/*psensor status: 0=>off,1=>on,2=>near,3=>far*/
...
static int syn_probe_init(void *arg)
{
	...
	if (board_build_flag() == 1) { //MFG_BUILD: 1 (Please double check MFG_BUILD value in file Devices_htc.c, if MFG_KERNEL_TOUCH_FW_UPDATE defined)
	...
	if (board_mfg_mode() == 5/*MFG_MODE_OFFMODE_CHARGING*/) {
	...
	//ts->debug_log_level |= BIT(0) | BIT(1);
