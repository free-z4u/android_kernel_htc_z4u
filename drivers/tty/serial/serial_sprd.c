...
static struct work_struct lock_off_w; /* work for actual lock off */
static struct workqueue_struct *hsuart_wq; /* hsuart workqueue */

static struct hrtimer lock_off_timer;  /* to poll TXEMT before lock off */
static ktime_t lock_off_delay;

static unsigned char suspended_flag=0;    /* 1 == suspended  0 == resumed */

static unsigned char uart0_status_flag=0;  /* 1= open, 0 = close */
...
#if defined(CONFIG_MACH_Z4DTG) || defined(CONFIG_MACH_Z4TD) || defined(CONFIG_MACH_CP5DTU) || defined(CONFIG_MACH_CP5DUG) || defined(CONFIG_MACH_CP5DWG) || defined(CONFIG_MACH_CP5VEDTU) || defined(CONFIG_MACH_CP5VEDWG)
...
static int bt_gpio_ctrl_config(void)
{
	...
	lock_off_delay = ktime_set(0, 150000000);  /* 150ms */
...
static int __init serial_sprd_console_setup(struct console *co, char *options)
{
	...
	/* TODO:
	 * CP5&Z4 use ttyS1 as console
	 * Add compatible codes here to fix another solution
	 *
	 * */
	if (co->index == 1) {
...
static int serial_sprd_resume(struct platform_device *dev)
{
    ...
    /*Set the clock again to avoid    BT open fail after deepsleep    issue 20130626*/
    if (0 == id)
    {
