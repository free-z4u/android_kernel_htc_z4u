...
static irqreturn_t dispc_isr(int irq, void *data)
{
	...
	//pr_debug("[DISP] dispc_isr (0x%x)\n",reg_val );
	//printk(KERN_DEBUG "[DISP][DEBUG] dispc_isr (0x%x)\n", reg_val );
	...
	if((reg_val & 0x2) && (SPRDFB_PANEL_IF_DPI ==  dev->panel_if_type)){ /*dispc external TE isr*/
		...
		if(0 != dev->esd_te_waiter){
			//pr_debug("[DISP] sprdfb:dispc_isr esd_te_done!\n");
			dev->esd_te_done =1;
...
{
	pr_debug(KERN_INFO "[DISP] sprdfb:[%s]\n",__FUNCTION__);
#if defined (CONFIG_MACH_CP5DTU) || defined(CONFIG_MACH_CP5DUG) || defined(CONFIG_MACH_CP5DWG) || defined(CONFIG_MACH_Z4TD)
	dispc_ctx.firstswitch = 0;
	dispc_ctx.lastcolor = SPRD_DATA_TYPE_LIMIT;
#endif
	/*set line buffer threshold at high level*/
