...
static void setup_autopd_mode(void)
{
	...
	//if(!(__raw_readl(REG_AON_APB_CHIP_ID)))
	if (soc_is_scx35_v0())
	...
	//sci_glb_set(REG_AP_AHB_MCU_PAUSE, BIT_MCU_SLEEP_FOLLOW_CA7_EN); // removed by htc/oliver
	sci_glb_write(REG_PMU_APB_AP_WAKEUP_POR_CFG, 0x1, -1UL);

#if 0   //remove by cuke move to the hboot
	// added by htc/oliver
	// XTLx_REL_CFG
	#if defined(CONFIG_MACH_Z4TD) 
	...
	// RF0/RF1
	sci_adi_clr(ANA_REG_GLB_PWR_XTL_EN0, BIT_LDO_RF0_XTL0_EN   | BIT_LDO_RF0_XTL1_EN   | BIT_LDO_RF0_XTL2_EN);   // BIT[0][1][2]
	sci_adi_set(ANA_REG_GLB_PWR_XTL_EN0, BIT_LDO_RF1_XTL0_EN   | BIT_LDO_RF1_XTL1_EN);   // BIT[6][7][8]
	// RF2/VDD25
	sci_adi_set(ANA_REG_GLB_PWR_XTL_EN1, BIT_LDO_RF2_XTL0_EN   | BIT_LDO_RF2_XTL1_EN);   // BIT[0][1][2]
	sci_adi_set(ANA_REG_GLB_PWR_XTL_EN1, BIT_LDO_VDD25_XTL0_EN | BIT_LDO_VDD25_XTL1_EN); // BIT[6][7][8]
	// DCDCCORE/DCDCWRF
	sci_adi_set(ANA_REG_GLB_PWR_XTL_EN4, BIT_DCDC_CORE_XTL0_EN | BIT_DCDC_CORE_XTL1_EN); // BIT[0][1][2]
	sci_adi_set(ANA_REG_GLB_PWR_XTL_EN4, BIT_DCDC_WRF_XTL0_EN  | BIT_DCDC_WRF_XTL1_EN);  // BIT[12][13][14]
	...
	#if defined(CONFIG_MACH_Z4TD)
		sci_adi_set(ANA_REG_GLB_LDO_PD_CTRL,  BIT_DCDC_WPA_PD);  //there is no use on z4td ,do forece pd it
	#else
	...
	sci_glb_write(REG_AON_APB_RES_REG0, BIT(8) | BIT(9), BIT(8) | BIT(9)); // [PATCH 01/12] bug#171338 set bb_bg bb_ldo auto enable
...
void disable_ahb_module(void)
{
	...
	//sci_glb_clr(REG_AP_AHB_AHB_EB, 0x1fff); // removed by htc/oliver

	// AP_PERI_FORCE_SLP
	//sci_glb_set(REG_AP_AHB_AP_SYS_FORCE_SLEEP_CFG, BIT_AP_PERI_FORCE_SLP);  //removed by cuke wang
	//sci_glb_set(REG_AP_AHB_MCU_PAUSE, BIT_MCU_SLEEP_FOLLOW_CA7_EN); // removed by htc/oliver
...
void disable_apb_module(void)
{
	...
	stat &= 0xf<<19 ; // just leave intc on,the uart will block entry AP_APB_SLEEP cuke
...
void disable_aon_module(void)
{
	...
	sci_glb_clr(REG_AON_APB_APB_EB0, BIT_CA7_DAP_EB); // added by htc/oliver
...
void disable_ana_module(void)
{
	...
	sci_adi_set(ANA_REG_GLB_LDO_PD_CTRL, BIT_LDO_SD_PD |  BIT_LDO_SIM2_PD | BIT_LDO_CAMA_PD |\
...
void bak_restore_ana(int bak)
{
	...
	//mask = BIT_LDO_SD_PD | BIT_LDO_SIM0_PD | BIT_LDO_SIM1_PD | BIT_LDO_SIM2_PD | BIT_LDO_CAMA_PD |\
		//BIT_LDO_CAMD_PD | BIT_LDO_CAMIO_PD | BIT_LDO_CAMMOT_PD | BIT_LDO_USB_PD | BIT_DCDC_WPA_PD;
	mask = BIT_LDO_SD_PD | BIT_LDO_SIM2_PD |  BIT_LDO_CAMA_PD |\
        BIT_LDO_CAMD_PD | BIT_LDO_CAMIO_PD | BIT_LDO_CAMMOT_PD | BIT_DCDC_WPA_PD;
	if(bak){
...
void int_work_round(void)
{
	...
	//sci_glb_set(SPRD_LPDDR2_PHY_BASE + 0x2c, BIT(4)); // removed by htc/oliver
	//sci_glb_write(SPRD_LPDDR2_BASE + 0x30, 0, -1UL);
	//sci_glb_set(SPRD_PMU_BASE + 0xa8, 1<<1);
	//sci_adi_clr(ANA_EIC_BASE + 0x18, 0x20);//not to control the headset interrupt enable
}
...
int deep_sleep(int from_idle)
{
	...
	if(!from_idle){
		...
		//disable_ahb_module(); // move below, htc/oliver -->
		//sci_glb_set(SPRD_PMU_BASE+0x00F4, 0x3FF);//remove by cuke,not control here,spl disable ddr low power mode 1) cause ddr pll always on consequently up 15ma 2)dsp hang
		disable_dma();
		//disable_mm();
		//disable_ana_module();
		disable_aon_module();
	...
	if(!from_idle){
		...
		//sci_glb_clr(SPRD_PMU_BASE+0x00F4, 0x3FF); //remove by cuke,not control here,spl disable ddr low power mode 1)ddr pll always on consequently up 15ma 2)dsp hang
		RESTORE_GLOBAL_REG;
	}
...
int sprd_cpu_deep_sleep(unsigned int cpu)
{
	...
	*/
	status = 0;
#endif
	/* sipc suspend flag is used for debug cp wake up ap int src*/
	sipc_suspend = 1;
	...
	if (status & DEVICE_AHB)  {
		...
		pm_enable_flag();
		//NOTE: judge if or not DDR clk has been setted to 200M.
		//If not Use BUG_ON to report it.
#if 1
...
void sc_pm_init(void)
{
	...
	//printk("power off %pf, restart %pf\n", pm_power_off, arm_pm_restart);
	init_gr();
	setup_autopd_mode();
	pm_ana_ldo_config();
	init_led();
