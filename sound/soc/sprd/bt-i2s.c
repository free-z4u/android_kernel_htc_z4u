...
#include <asm/io.h>
#include <linux/gpio.h>  //HTC_AUD add by zm  switch to fm or via
	...
	case SNDRV_CTL_IOCTL_VIA_FM_I2S3_SWITCH:
		...
		if (val & ~0x7) { /* read */
			...
			if(1 == val){
				...
				gpio_direction_output(AUD_VIA_FM_SEL, 0); //Switch to via
			}
			else if(4 == val)
				gpio_direction_output(AUD_VIA_FM_SEL, 1);  //Switch to fm
