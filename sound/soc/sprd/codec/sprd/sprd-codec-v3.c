...
//uint32_t sprd_get_vbat_voltage(void); //HTC_AUD_DEL by Evan.Xu  set volt=3800 due to power driver not ready
...
{
	int i;
	//HTC_AUD_START by Evan.Xu  set volt=3800 due to power driver not ready
...
static int sprd_codec_digital_open(struct snd_soc_codec *codec)
{
	...
	sprd_codec_dbg("Entering %s\n", __func__);
	snd_soc_update_bits(codec, SOC_REG(AUD_DAC_CTL), BIT(15), 0); // fix phone call downlink no sound ,the patch is from sprd zhenfang.wang
	sprd_codec_sample_rate_setting(sprd_codec);
			   ...
			   struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_update_bits(codec, SOC_REG(DCR2_DCR1), BIT(DIFF_EN),
				    BIT(DIFF_EN));
#endif
		/*set DAC_SDM_DODVl to 0x7 from weifeng.ni */
...
static struct snd_soc_dai_ops sprd_codec_dai_ops = {
	...
	//.digital_mute = sprd_codec_digital_mute, //patch for call mute fmsound
};
