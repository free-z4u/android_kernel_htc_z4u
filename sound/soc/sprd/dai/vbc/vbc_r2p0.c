...
#include <linux/clk.h>
#include <linux/io.h>
//HTC_AUD_START zanmin add I2S3 seclect vbc for extern fm
...
#include <linux/gpio.h>     // HTC_AUD:add by watson for i2s switch
...
static int fm_sample_rate = 48000;     // HTC_AUD modify by zanmin
struct st_hpf_dg {
	int hpf_switch[2];
	int dg_val[2];
	int hpf_val[2];
...
static inline int vbc_ad_enable(int enable, int chan)
{
	if (enable){
		...
		pr_info("cann't close ad chan.");/*dfm cann't close adc when FM chip is on*/
	}
	return 0;
}
...
//HTC_AUD_START zanmin add I2S3 seclect vbc for extern fm
	if(SND_SOC_DAPM_EVENT_ON(event)){
		...
		i2s3_sys_sel = (i2s3_sys_sel & ~(7 << 15)) | (4 << 15);   //FM radio, i2s3--->vbc
		__raw_writel(i2s3_sys_sel, CTL_PIN_BASE + REG_PIN_CTRL3);
                vbc_eq_setting.codec_dai = 1;
                vbc_eq_try_apply(1, VBC_CHAN_DA);  	 //apply eq setting for FM
#if defined (CONFIG_MACH_CP5DWG) || defined(CONFIG_MACH_CP5VEDWG)
#define AUD_FM_SEL	(135)
		gpio_direction_output(AUD_FM_SEL, 1);
#endif
	}/* else {
		uint32_t i2s3_sys_sel = __raw_readl(CTL_PIN_BASE + REG_PIN_CTRL3);
		i2s3_sys_sel = (i2s3_sys_sel & ~(7 << 15)) | (1 << 15); //CDMA 	phone call,i2s3 --->cp0
		__raw_writel(i2s3_sys_sel, CTL_PIN_BASE + REG_PIN_CTRL3);
...
static int vbc_eq_loading(struct snd_soc_codec *codec)
{
	...
	old_num_ad23 = vbc_eq_setting.hdr.num_ad23;
        ...
        //HTC_AUD_START evan_xu: fix ramdump when vbc_eq not correct
